...
resource "google_compute_region_network_endpoint_group" "p1_psc_neg_service_attachment" {
  name                  = "p1-psc-neg"
  project               = google_project.consumer.project_id
  region                = local.region-a
  network_endpoint_type = "PRIVATE_SERVICE_CONNECT"
  psc_target_service    = google_compute_service_attachment.p1_psc_service_attachment.id
  
  network    = google_compute_network.consumer_vpc_network.name
  subnetwork = google_compute_subnetwork.consumer_sb_subnet_a.self_link
}

resource "google_compute_region_network_endpoint_group" "p2_psc_neg_service_attachment" {
  name                  = "p2-psc-neg"
  project               = google_project.consumer.project_id
  region                = local.region-a
  network_endpoint_type = "PRIVATE_SERVICE_CONNECT"
  psc_target_service    = google_compute_service_attachment.p2_psc_service_attachment.id
...
#  region                          = local.region-a
  ...
  backend {
    group           = google_compute_region_network_endpoint_group.p1_psc_neg_service_attachment.id
